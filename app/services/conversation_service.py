    ...
    def create_conversation(
        ...
        while retry_count < max_retries:
            try:
                ...
                if ("duplicate" in error_msg or "unique" in error_msg or "conflict" in error_msg) and retry_count < max_retries:
                    # 새로운 UUID로 재시도
                    import time
        ...
        while retry_count < max_retries:
            try:
                ...
                try:
                    ...
                    print(f"대화 updated_at 업데이트 오류 (치명적이지 않음): {update_error}")
    ...
    def get_user_conversations(
        ...
        try:
            ...
            if conversation_ids:
                try:
                    # 모든 관련 메시지를 한 번에 조회
                    all_messages_result = self.supabase.table("messages").select(
                        "conversation_id, content, created_at"
                    ).in_("conversation_id", conversation_ids).eq("role", "user").order(
                    ...
                    if all_messages_result.data:
                        ...
                        for msg in all_messages_result.data:
                            ...
                            if conv_id and conv_id not in seen_conversations:
                                ...
                                first_messages_map[conv_id] = content[:50] + ("..." if len(content) > 50 else "")
                    ...
                    for conv_id in conversation_ids:
                        try:
                            msg_result = self.supabase.table("messages").select("content").eq(
                                "conversation_id", conv_id
                            ).eq("role", "user").order("created_at", desc=False).limit(1).execute()
